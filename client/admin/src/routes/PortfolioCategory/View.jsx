...
class Dynamic extends React.Component {
  ...
  delete = (id) => {
    axios.delete('/api/v2/portfolio/portfolioCategory', { data: { id } }).then((res) => {
      const {
        data: { message },
        statusText,
      } = res;
    ...
    componentWillMount = () => {
      axios.get('/api/v2/portfolio/portfolioCategory').then((result) => {
        ...
        data.map((element) => {
          element.action = (
            <span>
              <Link to={`/admin/portfolioCategory/${element.id}`} className="icon icon-feedback" />
          ...
          element.name = (
            <Link to={`/admin/portfolioCategory/${element.id}`}>{element.name}</Link>
          );
          return element;
        });
        this.setState(() => ({ data }));
