...
componentDidMount = async () => {
  ...
  const { data } = res;
  const {
    facebook, twitter, whatsapp, google, coloured_logo, email, address, youtube, instagram, linkedin,
  } = data[0];
  this.setState({
    facebook, twitter, coloured_logo, whatsapp, google, email, address, youtube, instagram, linkedin,
  });
}
      ...
      if (!err) {
        ...
        axios.post('/api/v2/options', values).then((result) => {
          ...
          if (result.status === 200) {
            ...
            setTimeout(() => {
              this.props.history.push('/admin/Setting/social');
  ...
  removeFile = async (file) => {
    if ((file.status = 'removed')) {
      ...
      await axios.post('/api/v2/files/removeFile', { pic }).then(() => {
  ...
  render() {
    ...
    const {
      facebook, twitter, whatsapp, google, youtube, instagram, disable, linkedin, email, address,
    } = this.state;
    ...
    return (
      <Card className="gx-card" title="Social Media And Adress">
