...
class Registration extends Component {
  ...
  componentDidMount = async () => {
    const result = await axios.get('/api/v2/titles');
    const { data } = result;
    const {
      featurestitle, featuressub, featuresimage,
    } = data[0];
    const fileList = [];
    if (featuresimage !== '') {
      await axios
        .get(`/api/v2/files/getFile/${featuresimage}`)
        .then(result => fileList.push({
          uid: uuid(),
          name: 'image.png',
          status: 'done',
          url: `/api/v2/files/getFile/${featuresimage}`,
        }))
        .catch((error) => {});
    }
    this.setState(() => ({
            ...
            .then((result) => {
              ...
              if (status === 200) {
                this.setState({ loading: false }, () => {
                  ...
                  setTimeout(() => {
                    this.props.history.push('/admin/features/settings');
  ...
  render() {
    ...
    const {
      featurestitle, featuressub, fileList,
      featuresimage,
      previewVisible,
      disable,
    } = this.state;
    ...
    return (
      <Card className="gx-card" title="Features Settings">
            ...
            <>
              ...
              >
                <img
                  alt="example"
                  style={{ width: '100%' }}
                  src={`/api/v2/files/getFile/${featuresimage}`}
                />
              </Modal>
            </>
          </FormItem>
