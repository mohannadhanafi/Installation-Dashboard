...
class Dynamic extends React.Component {
  ...
  delete = (id) => {
    axios.delete('/api/v2/blogs/comments', { data: { id } }).then((res) => {
  ...
  update = (id) => {
    axios.post('/api/v2/blogs/comments/update', { data: { id } }).then((res) => {
      const { data: { message }, statusText } = res;
      if (res.status === 200) {
        const { data } = this.state;
  ...
  getData = () => {
    this.setState({ loading: true }, async () => {
      axios.get('/api/v2/blogs/comments').then((result) => {
        const { data } = result;
        const columns = [
          {
            title: 'Email',
          ...
          },
          {
            ...
            dataIndex: 'blog',
            key: 'blog',
            sorter: (a, b) => a.blog.props.children.length - b.blog.props.children.length,
          }, {
            title: 'Status',
            dataIndex: 'approve',
            key: 'Status',
        ...
        data.map((element) => {
          ...
          element.blog = (
            <Link to={element.post ? `/admin/blogs/${element.post.id}` : 'posts'}>
              {element.blog && element.blog.title.slice(0, 50)}
            </Link>
          );
          return element;
        });
