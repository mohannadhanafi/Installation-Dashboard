...
class Registration extends Component {
  ...
  componentDidMount() {
    ...
    axios.get(`/api/v2/portfolio/portfolioCategory/${id}`).then(async (result) => {
      const { data } = result;
      const { name } = data;
      this.setState({ name });
  ...
  handleSubmit = (e) => {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        axios
          .post(`/api/v2/portfolio/portfolioCategory/update/${id}`, values)
          .then((result) => {
            const {
              data: { message },
            } = result;
            ...
            setTimeout(() => {
              this.props.history.push('/admin/portfolioCategory/view');
