    ...
    componentWillMount = () => {
      axios.get('/api/v2/portfolio').then((result) => {
        ...
        const columns = [
          ...
          {
            title: 'Category',
            dataIndex: 'porfolio_category.name',
            key: 'porfolio_category',
            sorter: (a, b) => a.porfolio_category.length - b.porfolio_category.length,
          },
          {
            title: 'Action',
            key: 'action',
        ...
        data.map((element) => {
          element.action = (
            <span>
              <Link to={`/admin/portfolio/${element.id}`} className="icon icon-feedback" />
