...
class Registration extends Component {
  ...
  componentDidMount = async () => {
    ...
    const result = await axios.get(`/api/v2/features/${id}`);
    const { data } = result;
    const {
      sub_title, title, cta, desc, icon, body,
    } = data[0];
    const fileList = [];
    if (icon !== '') {
      await axios
    ...
    this.setState({
      sub_title, title, cta, desc, icon, fileList, content: body,
  ...
  render() {
    const {
      title, cta, desc, icon, previewVisible,
      fileList, disable,
      content,
    } = this.state;
